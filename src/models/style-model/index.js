  ...
  return {
    ...
    label: {
      value: {
        getStyle: () => ({
          ...
          colorExpression:
            findComponent("label")?.label?.value?.colorExpression ?? layout?.style?.fontColor?.colorExpression ?? "",
        }),
      },
    },
    backgroundColor: {
    ...
    border: {
      getStyle: () => ({
        ...
        fullBorder:
          findComponent("border")?.border?.fullBorder ?? layout?.style?.border?.fullBorder ?? DEFAULTS.BORDER_FULL,
        colorType:
          findComponent("border")?.border?.colorType ?? layout?.style?.border?.colorType ?? DEFAULTS.BORDER_COLOR_TYPE,
        color:
          findComponent("border")?.border?.color?.color ??
          layout?.style?.border?.color?.color ??
          DEFAULTS.BORDER_COLOR.color,
        colorExpression:
          findComponent("border")?.border?.colorExpression ?? layout?.style?.border?.colorExpression ?? "",
      }),
    },
    image: {
      getStyle: () => ({
